<div>
	...
	<form wire:submit="save">
											...
											<div class="col relative border shadow rounded-2xl overflow-hidden"
												...
												<div
													...
													<div class="flex items-center gap-2 justify-end">
														<button data-fg-toggle="modal" data-fg-target="#edit-slide-{{ $slide['id'] }}"
		...
		@foreach ($form['time_slots'] as $ti => $ts)
		@foreach ($ts['slides'] as $si => $sl)
		<div id="edit-slide-{{ $sl['id'] }}" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title">Edit slide {{ $sl['id']}}</h5>
					...
					<div class="modal-body">
						<div class="grid grid-cols-1 gap-3">
							<div class="col">
								<fgx:input
									wire:model.live="form.time_slots.{{ $ti }}.slides.{{ $si }}.name"
									id="form.time_slots.{{ $ti }}.slides.{{ $si }}.name"
									:label="__('Name')" />
							</div>
							<div class="col">
								<fgx:input type="number"
									wire:model.live="form.time_slots.{{ $ti }}.slides.{{ $si }}.duration"
									id="form.time_slots.{{ $ti }}.slides.{{ $si }}.duration"
									:label="__('Duration')" />
							</div>
							<div class="col">
								<fgx:input
									wire:model.live="form.time_slots.{{ $ti }}.slides.{{ $si }}.transition"
									id="form.time_slots.{{ $ti }}.slides.{{ $si }}.transition"
									:label="__('Transition')" />
							</div>
							<div class="col">
								<fgx:switch
									wire:model.live="form.time_slots.{{ $ti }}.slides.{{ $si }}.is_active"
									id="form.time_slots.{{ $ti }}.slides.{{ $si }}.is_active"
									:label="__('Active')" checked="{{ boolval($sl['is_active']) }}"/>
							</div>
						</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-primary" data-fg-dismiss="modal">Done</button>
					</div>
				</div>
			</div>
		</div>
